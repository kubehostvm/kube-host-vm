...
qemu-system-x86_64 \
  ...
  -nographic \
  -vnc :0 \
  -netdev tap,id=net0,script=/etc/kube-host-vm/tap-into-ovs.sh,downscript=/etc/kube-host-vm/untap-into-ovs.sh,vhost=on \
  -device virtio-net-pci,netdev=net0,id=nic0,mac="${MAC_ADDR}" \
